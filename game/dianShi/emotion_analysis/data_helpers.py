...
def load_positive_negative_data_files(positive_data_file, negative_data_file, mid_data_file):
    """
    ...
    mid_examples = read_and_clean_zh_file(mid_data_file)
    # Combine data
    x_text = positive_examples + negative_examples+mid_examples
    # Generate labels

    negative_labels = [[0, 0,1] for _ in negative_examples]
    mid_labels = [[0,1, 0] for _ in mid_examples]
    positive_labels = [[1, 0, 0] for _ in positive_examples]
    y = np.concatenate([positive_labels, negative_labels,mid_labels], 0)
    return [x_text, y]

def load_test_files(test_file):
    """
    Loads MR polarity data from files, splits the data into words and generates labels.
    Returns split sentences and labels.
    """
    # Load data from files
    test_file_examples = read_and_clean_zh_file(test_file)

    # Combine data
    x_text = test_file_examples
    # Generate labels

    test_labels = [[0, 0,0] for _ in test_file_examples]
    y = np.concatenate([test_labels], 0)
    return [x_text, y]
